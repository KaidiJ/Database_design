...
class cc_sql:
    ...
    def callkaidi(self, user_input, limit=None, condition_str=None, columns_str=None):
        ...
        #Command: delete_table table_name
            ...
            _, dbname = user_input.split()
            return self.db.delete_table(dbname)
        ...
        #value is the specified value
            ...
            condition = self.db.create_condition(key, float(value) if self.is_float(value) else int(value) if value.isdigit() else value, operator)
        ...
        #Command: groupby table_name new_table_name groupby_column
            ...
            agg_response = prompt('db> Using aggregation? (e.g. max/min/sum/avg column_name new_tablename or count new_tablename or no): ')
            if agg_response.lower() == 'no':
                show_response = prompt('db> Show data? (yes/no): ')
                if show_response.lower() == 'yes':
                    limit_response = prompt('db>show> How many rows do you want to present? (type an interger/no): ')
                    limit = None
